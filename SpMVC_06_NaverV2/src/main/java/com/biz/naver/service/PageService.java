...
public class PageService {
	...
	public PageDTO makePagiNation(long totalCount, long currentPageNo) {
		...
		// (전체데이터 + 보여질리스트 -1 / 보여질 리스트
		long finalPageNo = totalCount + (totalCount + (listPerPage -1 )) / listPerPage;
		...
		// 이전, 이후를 계산하기 위해서 현재 페이지번호가 
		// 첫페이지인가, 마지막 페이지인가를 검사해서 flag를 세팅
		boolean isNowFirst = currentPageNo == 1;
		boolean isNowFinal = currentPageNo == finalPageNo;
		
		// 하단에 리스트로 보여줄 페이지 계산
		// cur : 3 이면 1~5 cur 10이면 8~12까지 이런식으로 보여주기위한 계산
		...
		PageDTO pageDTO = PageDTO.builder().totalCount(totalCount).pageCount(this.pageCount).listPerPage(listPerPage)
				.firstPageNo(1).finalPageNo(finalPageNo).endPageNo(endPageNo).startPageNo(startPageNo).currentPageNo(currentPageNo).build();
