...
<body>
<%@ include file="/WEB-INF/views/include/include-dept-header.jsp" %>
<form method="POST">
	
	<fieldset>
	<legend>거래처정보입력</legend>
	

	<!-- label의 for 속성 : input box와 한 그룹으로 설정,
	label을 클릭 했을 때 마치 input box를 클릭 한 것처럼 input box에 focus를 지정 -->
	<label for="d_code" class="in-lable">거래처코드</label>
	<div class="in-box">
		<input name="d_code" id="d_code" value="${DI.d_code}" readonly="readonly">
	</div>	
	
	<label for="d_name" class="in-lable">거래처명</label>
	<div class="in-box">
		<input name="d_name" id="d_name" value="${DI.d_name}">
	</div>	
	
	<label for="d_ceo" class="in-lable">대표</label>
	<div class="in-box">
		<input name="d_ceo" id="d_ceo" value="${DI.d_ceo}">
	</div>	
	
	<label for="d_tel" class="in-lable">전화번호</label>
	<div class="in-box">
		<input name="d_tel" id="d_tel" value="${DI.d_tel}">
	</div>	

	<label for="d_addr" class="in-lable">주소</label>
	<div class="in-box">
