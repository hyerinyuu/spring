	...
	*/
	...
	var img_context = function(key, option){
		...
		if(key == 'delete'){
			...
			window.open("${rootPath}/rest/file_down/" + img_file_seq, target="_new")
			...
			// 현재 클릭된(div) tag가 포함하고 있는 tag중 <img>라는 tag의 src값("이미지 이름")을 찾아서 img_src로 선언
			let img_src = $(this).find("img").attr("src")
			
			// 전체 src값에서 파일 이름만 추출하기 위해서
			let img_srcs = img_src.split("/") 			// "/"를 기준으로 문자 분해
			let img_srcs_len = img_srcs.length
			let img_file = img_srcs[img_srcs_len - 1]  // 분해된 배열의 제일 마지막 값을 가져오면 파일 이름만 추출 완료
			
			$.post(
					"${rootPath}/rest/main_image",
					// 보낼 값(imng_pcode에 img_pcode를 실어서 보냄)
					{img_pcode : img_pcode, img_file : img_file},
					// 호출이 성공적으로 이루어지면 function이 받음
					function(result){
						// 페이지 새로고침
						document.location.replace(document.location.href)						
					}
			// ajax와 다르게 post가 끝나는 지점에 error message 표시해야함
			).error(function(){
				alert("통신 오류")
			})
