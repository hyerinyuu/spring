...
public class ProductController {
	...
	public String input(@ModelAttribute ProductDTO proDTO, @RequestParam("u_file") MultipartFile u_file) {
		...
		log.debug("업로드한파일 : " + u_file.getOriginalFilename());
		
		
		// 파일을 선택하지 않았으면 파일에 관한 처리를 수행하지마라
		// 
		// (파일이 업로드되어있는 상태에서 아무일을 하지 않은채 저장을 누르면 기존 이미지가 사라지는 버그 막기위해)
		if(!u_file.isEmpty()) {

			// update를 수행할 때
			// 이미 업로드된 파일이 있으면 기존의 파일을 삭제하고 새로운 파일을 업로드 해야하므로
			// p_file 변수를 검사하여 값이 있으면 파일을 삭제하기
			...
			/*
			 * web에서 파일이 전송되어 오면
			 * 1. fService.fileUp() method에게 파일을 어딘가에 저장해달라 라고 요청(복사)
			 */
			String upFileName = fService.fileUp(u_file);
			if(upFileName != null) {
				
				// 2. 정상적으로 저장이 완료되면
				// 파일이름을 DTO의 p_file변수에 저장함.
