...
<article>
	<ul class="pagination">
		...
		<c:if test="${pageDTO.startPageNo > 1}">
			<li class="page-item"><a class="page-link" href="${rootPath}/${url}?search=${search}&currentPageNo=1">1</a></li>
			<li class="page-item"><a class="page-link" href="${rootPath}/${url}?search=${search}&currentPageNo=${pageDTO.prePageNo}">&lt;</a></li>
		</c:if>
		...
		<c:forEach begin="${pageDTO.startPageNo}" end="${pageDTO.endPageNo}" var="pageNo">
			<%
			// 현재페이지번호와 pageNo가 일치하면 active해라
			//()
			%>
